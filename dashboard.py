...
def get_process_time(output):
    ...
    for result in results:
        ...
        sys.exit(0)
    pidInfo = [result.decode('utf-8').split()[1] for result in results
        if result.decode('utf-8').split()[0] == pid][0]
...
def get_num_videos_played():
    if os.path.isfile(abs_path + '.count_video'):
...
def get_num_playlist_playthroughs():
    if os.path.isfile(abs_path + '.count_playlist'):
...
def update(video_title):
    with open('/tmp/motd.tmp', 'w') as dashboard_text:
        dashboard_text.write('FluxVision\n\n')
        dashboard_text.write('Uptime: ' + get_process_time("duration") + '\n')
        dashboard_text.write('Started: ' + get_process_time("start") + '\n')

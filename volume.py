...
def dbus_setup():
    ...
    while done == 0:
        try:
            ...
            omxplayer_pid = int(omxplayer_pid.decode('utf-8').strip())
            proc_stat_file = os.stat(f"/proc/{omxplayer_pid}")
            ...
            #set effective user and group to omxplayer dbus process owner
            os.setegid(group_id)
            os.seteuid(user_id)
